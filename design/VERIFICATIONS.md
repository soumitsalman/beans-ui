...
- Top Headlines is a horizontal carousel that fetches its next page when its actual carousel API reaches the final scroll snap; Latest News is a vertical list with a `More` control directly beneath its loaded rows whenever a cursor exists.
...
- News cards link to the matching story. Story detail shows the story metadata, the longest available summary paired with its title, propagation, and source coverage.
- Coverage is latest-first, independently cursor-paged in groups of five, and each article URL is actionable.
- Coverage rows show their source label, humanized article mention count when present, and linked title without inventing a count when mentions are absent.
...
- The application shell exposes the canonical `/`, `/categories/{category_slug}`, `/stories/{story_id}`, and `/search` routes. Its header shows the current date, Beans mark, Search control, Beans API control, and Contact control; its footer exposes every specified Cafecito, Publications, API, GitHub, and About link.
...
- Top Headlines is rendered as a vertical list, Latest News as a carousel, carousel-end loading checks an unavailable nested ref instead of the exposed carousel API, or the Latest News `More` control is absent/buried away from the end of the list while a cursor exists.
...
| Coverage mentions | Coverage articles with present and absent mention values | The detailed story loads | Each available mention count is compactly humanized beside its source label; unavailable values are omitted. |
...
| Shell and canonical routes | A mobile viewport | The header, category navigation, footer, and each named route are opened | The required date, Beans mark, Search/API controls, exact footer destinations, and `/categories/*` and `/stories/*` routes are present and usable. |
...
| Header Contact and 320px chrome | A 320px viewport and the documented Contact URL | The header renders | Contact opens `https://developer.cafecito.tech/contact` with `noopener noreferrer`; the header still shows the date, Beans mark, and Search, API, and Contact controls. |
...
| Carousel snap continuation | A one-card-in-view Top Headlines carousel with another cursor page | The user reaches the final snap | `canScrollNext() === false` requests the next cursor; the snap stays one card wide. |
...
- Manual UI checks: inspect the 320px viewport, a wider mobile viewport, header/footer destinations, carousel-end loading, latest-news load more, all search modes, story-less and story navigation, Coverage links, loading/empty/error states, and dark-theme contrast.
