...
- [WORKLOG](./design/WORKLOG.md): Maintain through changes
- [VERIFICATIONS](./design/VERIFICATIONS.md): Success criteria and failure modes.
- Every implementation: think through success criteria and failure modes -> build verification cases -> build guard-rails around failure modes -> implement solution -> verify

## API Backend & Data Sources
### Beans API
...
- Base URL: https://cafecito-beans-api.fly.dev | [Swagger Spec](./design/beans.openapi.yaml)
- Use `content_type=news` query param for news | Set `X-API-KEY` header value from env var `CAFECITO-API-KEY`
...
## Design
- Pages: Home (shows stories/articles of), Category (shows stories/articles from that category), Story (shows details of that story)
- Theme & Color: Modern, Minimalist, Dark Coffee + Dark Charcoal Gray, Matte, Dark Mode ONLY
...
- Trend Score: humaize using icons. fire >= 10000, trending_up >= 1000
- Source favicon: article.source.favicon or source.favicon or get_favicon(article.url) or default_source_favicon
- Source label: article.source.site_name or article.source.domain_name or base_url_without_scheme_prefix(article.url)
- Product Icons: ./public/*

### Home Page & Category Page
- Top Headlines: Carousel. Exclude if empty. Fetch more when user reaches end of view.
- Latest news: List. Exclude if empty. `More` button loads more items.
- top headlines and latest news: Deduplicate using story_id | Fetch limit=5 | Use next_cursor to fetch more when needed
- news item: exclude image if not exists - avoid default_news_image
- news item: click goes to story page
- news item: sources show <=5 favicons as avatar group + sources count

``` Top Headlines Item Layout
+-----------------------------------
| image if exists
+-----------------------------------
...
``` Coverage Item Layout
+--------------------------------------------
| source_favicon | source_label     mentions
|                | title
+--------------------------------------------
```
